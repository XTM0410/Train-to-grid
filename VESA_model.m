function [RTP,RTC,TDR] = VESA_model(ChangeofPower_MultiplePeriod,T2GDeployingPeriod_MultiplePeriod)
...
for tauG_count = 1:tauNum
    tauG = T2GDeployingPeriod_MultiplePeriod(tauG_count);
    ChangeofPower = ChangeofPower_MultiplePeriod{tauG_count};
    Duration = T2GDeployingPeriod_MultiplePeriod(tauG_count)/60;
    if tauG == 0
...
RTP = sum(Batterylist(1,:));
RTC = sum(Batterylist(2,:));
Tlist = 0:0.01:15;
