function [Power,Energy,ChangeofPower] = DTP_evaluation(TrainConfiguration,T2GSet)
%DTP_EVALUATION Deployable T2G power evaluation
% Input:
...
list_Trainindex = T2GSet{1};
delta_t = T2GSet{2};
T_T2G = T2GSet{3}; T_T2Gb = T_T2G(1); T_T2Ge = T_T2G(2); 
Strategy = T2GSet{4};
t_begin = T2GSet{5};
        ...
        for j=1:T_mode
            ...
            elseif j>taue
                ...
                if TractionEnergy>5*BrakingEnergy || BrakingEnergy<0.01 % traction dominates
                    ...
                    elseif Strategy == 2 || Strategy == 3
                        Constraints=[Constraints,0<=DPM(i,j)+PMij<=Pm]; 
                    else
                        error('Invalid T2GSet!')
                    end
